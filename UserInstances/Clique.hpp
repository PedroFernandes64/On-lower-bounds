...
class Clique
{
	private:
		...
		unsigned								index_;
		unsigned 								size_;
		unsigned								omega_;  						//sum of weights of composing vertices
		std::vector<forbiddenEdgesFromCliques>	forbiddenEdgesFromCliques_; 	
		std::vector<Path*>						paths_;							//Paths that originated clique

	public:
		//Constructors
		Clique(unsigned i = 0, unsigned s = 0, unsigned omega = 0): vertices_(), index_(i), size_(s), omega_(omega){}
		...
		const unsigned &								getOmega() const {return omega_;}
		...
		void							addPath(Path* & p);													//add path that originated the clique
